...
for model in ['ICHEC','CNRM','MPI']:
    ...
    for period in os.listdir(modeldir):
            ...
            cap_share_per_node = network.generators.p_nom_opt.groupby([network.generators.bus, network.generators.carrier]).sum() # per carrier and bus
            ...
            if cap_share_all is None:
                    ...
                    cap_share_per_node_all = pd.DataFrame(index=cap_share_per_node.index)
            ...
            cap_share_per_node_all[period] = cap_share_per_node
            gen_share_all[period] = gen_share
            gen_share_per_node_all[period] = gen_share_per_node

    # cap_share_all.to_csv("../data/cap_share_all_periods_model_{}.csv".format(model))
    cap_share_per_node_all.to_csv("../data/cap_share_per_node_all_periods_model_{}.csv".format(model))
    # gen_share_all.to_csv("../data/gen_share_all_periods_model_{}.csv".format(model))
    # gen_share_per_node_all.to_csv("../data/gen_share_per_node_all_periods_model_{}.csv".format(model))
